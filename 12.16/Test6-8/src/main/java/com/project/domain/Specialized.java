...
@Table(name= "if_dk_item_보도자료") //DB에서 생성할 Table의 이름
public class Specialized {
	@GeneratedValue(strategy = GenerationType.IDENTITY) //pk값을 지정하기 위해 @ID 어노테이션을 사용하여 생성 전략을 재정의
	@Id //pk를 나타내기 위해 @ID 어노테이션 사용
	public Long META_ID;
	@Column(name= "META_CLASSIFI", columnDefinition = "TEXT") //테이블의 컬럼 이름을 "META_CLASSIFI"로 컬럼의 크기를 1000으로 지정 
	public Long classifi;
	@Column(name= "META_TYPE", columnDefinition = "TEXT") //이하 동문
	public Long type;
